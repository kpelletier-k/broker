...
protected:
    ...
    virtual void on_session_open(const std::shared_ptr<BrokerSessionS>& session) = 0;
    virtual void on_session_close(const std::shared_ptr<BrokerSessionS>& session) = 0;

    virtual void on_log(const std::string& name, EventLogType type, const std::string& message) = 0;
    virtual bool on_message(const std::shared_ptr<BrokerSessionS>& session, const ProtoMessage& method, const LayerReplyFnc& reply_fnc) = 0; // return success
};
